<?php
/**
 * Plugin Name: Caldera Forms password fields
 * Plugin URI:  
 * Description: Password field for Caldera Forms
 * Version:     0.0.1
...
add_filter('caldera_forms_get_field_types', 'cf_password_field_register_function');
 ...
 * @return array $fields
 */
function cf_password_field_register_function($fields){
	...
	$fields['password'] = array(
		"field"				=>	__("Password", 'cf_password'),
		"file"				=>	plugin_dir_path( __FILE__ ) . 'field.php',
		"category"			=>	"Basic", 					// comma separated list of categories to place the field in
		"description" 		=>	__('Password field','cf_password'), 	// description explains what the field is for
		"viewer"			=>	'cf_password_viewer_function', // array($this, 'viewer_function')		// viewer function is used to processes the stored value for display purposes. i.e if saving a saving a post ID the viewer whould get the post title to display
		"handler"			=>	'cf_password_handler_function', // array($this, 'handler_function')		// handler function is used to processes the submitted value before storage. Like a file uploader to store the saved URL
 ...
 */
function cf_password_viewer_function($value, $field ,$form){
 ...
 */
function cf_password_handler_function($value, $field ,$form){
